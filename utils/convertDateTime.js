...
export const convertTimestampToDateAMPM = (timestamp, arrivalTime, stayDuration) => {
    // Create a Date object from the timestamp and convert it to IST
    const date = new Date(timestamp).toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
    const dateObj = new Date(date);
    ...
    // Format the arrival time to "HH:00:00"    
    const temp = String(Number(arrivalTime) > 12 ? Number(arrivalTime) - 12 : Number(arrivalTime))
    let formattedArrivalTime;
    if(arrivalTime == 0){
        formattedArrivalTime = "12:00 AM"
    }
    else if(arrivalTime > 0 && arrivalTime < 12){
        formattedArrivalTime = arrivalTime+":00 AM"
    }
    else if(arrivalTime == 12){
        formattedArrivalTime = "12:00 PM"
    }
    else{
        formattedArrivalTime = String(Number(arrivalTime)-12)+":00 PM"
    }
